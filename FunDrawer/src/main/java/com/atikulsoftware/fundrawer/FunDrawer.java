...
public class FunDrawer extends ElasticDrawer {

    public FunDrawer(Context context) {
        super(context);
    }

    public FunDrawer(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public FunDrawer(Context context, AttributeSet attrs, int defStyle) {
                    ...
                    if (mDrawerState == STATE_DRAGGING_OPEN) {
                        if (getPosition() == Position.LEFT) {
                            if (mOffsetPixels + dx < mMenuSize / 2) {
                                onMoveEvent(dx, y, FunMenuLayout.TYPE_UP_MANUAL);
                            } else {
                                mVelocityTracker.computeCurrentVelocity(1000, mMaxVelocity);
                                final int initialVelocity = (int) getXVelocity(mVelocityTracker);
                                mLastMotionX = x;
                        ...
                        onMoveEvent(dx, y, FunMenuLayout.TYPE_DOWN_MANUAL);
                    }
                }
                break;
            }
